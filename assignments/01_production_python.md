...
├── conftest.py             <- empty file; makes the package importable in tests (see below)
...
Create an **empty** file called `conftest.py` at the top of `assignments_01/`. You do not put anything in it. Its presence tells pytest to add `assignments_01/` to the list of places Python searches for modules, so that `from weatherkit import ...` works inside your tests when you run `pytest` from `assignments_01/`. Week 1's modules lesson explains why this is necessary: without it, pytest searches from the `tests/` directory, where `weatherkit/` is not visible, and every test fails to import with `ModuleNotFoundError: No module named 'weatherkit'`.
...
> Note: this is the **hourly** Open-Meteo response, which is shaped differently from the **daily** response used in the Pydantic lesson (the daily response has fields like `temperature_2m_max`). That is deliberate. The point of this task is to apply the same validation pattern to a response shape you have not seen before, which is exactly what happens with real APIs. The daily response is the one you will use for the machine learning weeks and the cloud pipeline later in the course.
...
Create the `weatherkit/` directory with an `__init__.py`, plus a `tests/` directory. Add the empty `conftest.py` at the root of `assignments_01/` described in the submission instructions above. Confirm you can run `pytest` from `assignments_01/` and have it collect zero tests without erroring.
...
## Extension A (Optional): Handle Missing Readings (Low)
...
## Extension B (Optional): Configurable Summaries (Moderate)
...
**General grading notes for this assignment:**

- **File paths and folder layout are enforced by the automated `pytest` run, not by inspection.** The reviewer cannot see the student's filesystem. Do not fail a student for a path or directory that you cannot verify (for example the location of `weather_raw.json`, or the exact internal split of the package into `schemas.py` / `records.py` / `summarize.py`). What matters is that the package imports and the `tests/` suite passes when `pytest` is run from `assignments_01/`.
- **Class, model, method, and function names given in the tasks are exact.** `Use exactly as written (later tasks and the tests depend on these names)`: `HourlyBlock`, `WeatherResponse`, `HourlyReading`, `to_readings`, `DailySummary`, `DailyAggregator`, `min_hours`, `summarize`, `incomplete_days`, `temp_range`, and the warmup names `Thermometer`, `TemperatureAlert`, `Station`, `StationBatch`, `Reading`, `celsius_to_fahrenheit`, `mean`.
- **Sample values are examples.** `Example — adapt to your own values`: any location, date, or temperature the student picks, and the `Thermometer(location='Charlotte', n_readings=4, average=18.6)` repr format (the assignment says "something like").

**Part 1 — `warmup_01.py`** (a single file holding every warmup answer). The warmup `pytest` functions live in this file and are run explicitly with `pytest warmup_01.py -v`; they are intentionally **not** collected by a bare `pytest` run of the `tests/` suite, so do not fail the student because `warmup_01.py` is not part of the discovered suite. Each item below includes the short explanatory comment the task asks for.

- **Classes Q1** — `Thermometer` class: `__init__(location, readings=[]-equivalent)`, `add`, `average` returning `None` when empty, `hottest` returning `None` when empty; a demo that adds ≥4 readings and prints average and hottest; a comment on why `average()` handles the empty case.
- **Classes Q2** — a `__repr__` on `Thermometer`; prints the object directly and a list of two; a comment on the default repr.
- **Classes Q3** — `TemperatureAlert` with `threshold` (default `30.0`) and `breaches(thermometer)`; two alerts run against the same `Thermometer`; a comment on why the threshold lives on the object.
- **Dataclass Q1** — `Station` rewritten as a dataclass with type hints and a docstring; an equality demo with a comment explaining the result.
- **Dataclass Q2** — frozen `Station`: `FrozenInstanceError` caught and printed (script does not crash); a `set` of three with two identical, length printed; a comment on what `frozen=True` adds.
- **Dataclass Q3** — `StationBatch` (`region`, `stations` list defaulting empty, `add`, `highest` returning `Station | None`); the mutable-default error captured in a comment, then fixed with `field(default_factory=...)`; type hints and docstrings on methods.
- **Pydantic Q1** — `Reading` model with the four constrained fields; one valid instance printed.
- **Pydantic Q2** — three separate failures each caught with `try`/`except ValidationError`; the coercion demo (`"21.5"` and integer `40`) with the resulting types printed; a comment stating the coercion rule.
- **Pydantic Q3** — one construction with several errors at once; a loop over `e.errors()` printing `loc` and `msg`; a comment on the error count and why reporting all at once helps.
- **Pydantic Q4** — a `model_validator(mode="after")` rejecting `humidity == 0.0` **and** `temperature_c < -40`; proof a valid reading still constructs and the bad combination raises; a comment on why `Field` alone cannot express this.
- **pytest Q1** — `celsius_to_fahrenheit` with a docstring and `test_celsius_to_fahrenheit` using `pytest.approx` for the 37 °C case; a comment on why `approx` was necessary.
- **pytest Q2** — `mean` raising `ValueError` on empty input and `test_mean_of_empty_raises` using `pytest.raises(ValueError, match=...)`; a comment on what `match=` adds.
- **pytest Q3** — `test_mean_values` with `@pytest.mark.parametrize` over ≥4 cases (including a single-element list and negatives); the pasted summary line; a comment on why one parametrized test beats four near-identical ones.
- **pytest Q4** — the function deliberately broken, the failure output pasted into a comment, then fixed; a comment on which values pytest showed.

**Part 2 — the `weatherkit` package**

- **Task 1 — Package skeleton.** `weatherkit/` with `__init__.py`, a `tests/` directory, and an **empty `conftest.py`** at the root of `assignments_01/`. `__init__.py` re-exports the main classes so `from weatherkit import ...` works. `pytest` run from `assignments_01/` collects zero tests without error.
- **Task 2 — `schemas.py` (the boundary).** `HourlyBlock` (`time: list[str]`, `temperature_2m: list[float]`, `precipitation: list[float]`) with a `model_validator(mode="after")` that rejects unequal list lengths; `WeatherResponse` (`latitude` constrained −90…90, `longitude` −180…180, `timezone`, `elevation`, nested `hourly: HourlyBlock`); docstrings on both. Validating `weather_raw.json` reports **168** hourly observations.
- **Task 3 — `records.py` (inside the boundary).** `HourlyReading` dataclass (`timestamp: str`, `temperature_c: float`, `precipitation_mm: float`) with a units docstring; `to_readings(response: WeatherResponse) -> list[HourlyReading]` preserving order, with an `Args:`/`Returns:` docstring; a comment explaining why this is a dataclass while `WeatherResponse` is a Pydantic model.
- **Task 4 — `summarize.py` (the aggregation).** `DailySummary` dataclass (`date`, `temp_max`, `temp_min`, `precipitation_sum`, `hours_observed`) with a `temp_range()` method; `DailyAggregator(min_hours: int = 24)` with `summarize(...)` (groups by the date portion of the timestamp, computes max/min/precip sum, records hours, drops days below `min_hours`, returns sorted by date) and `incomplete_days(...)` returning the dropped dates. Self-check produces **7 summaries**; the first two rows match the given table exactly (`2026-04-08 → 16.8 / 7.8 / 0.0 / 24`, `2026-04-09 → 19.3 / 3.7 / 0.0 / 24`) — these are exact expected outputs.
- **Task 5 — the test suite (must pass via `pytest` from `assignments_01/`).** `test_schemas.py` ≥4 tests (valid real JSON with 168 entries loaded via `Path(__file__).parent.parent / "weather_raw.json"`; `latitude=200.0` raises; mismatched list lengths raise; a `null` in `temperature_2m` raises); `test_records.py` ≥3 tests (one reading per hour in order; index-*i* pairing; value equality); `test_summarize.py` ≥5 tests using a `@pytest.fixture` (grouping across two dates; correct max/min; precip sum via `pytest.approx`; a short day dropped and present in `incomplete_days()`; lowering `min_hours` keeps that day). At least one test in the suite uses `@pytest.mark.parametrize`. Includes the comment naming which test caught a deliberately broken function.
- **Task 6 — `report.py`.** A `main()` under an `if __name__ == "__main__":` guard that loads, validates, converts, aggregates, prints a per-day table, and prints a warning listing dropped incomplete days; a comment on what omitting the guard would cause on import.
- **Task 7 — Reflection.** A comment block at the bottom of `report.py` answering the three questions (null-tolerance trade-off and the schema change for it; the half-day `min_hours` problem and how `incomplete_days()` helps; one concrete Week 10 benefit of a package over one file).

### Optional Deliverables/Tasks

**Do not fail a student for omitting any of these.** They are marked "(Optional)" in the assignment.

- **Extension A (Optional)** — `HourlyBlock` tolerates `None` in `temperature_2m`/`precipitation`; `to_readings` and `DailyAggregator` skip missing values while still counting hours correctly; tests cover a day with a gap.
- **Extension B (Optional)** — a `RunningWindow` class returning, per day, the start hour and length of the longest run of consecutive hours inside a configurable comfortable range with zero precipitation; boundary tests (runs at the start/end of a day, and a day with no qualifying hours).
- **Extension C (Optional)** — a minimal `pyproject.toml` and `uv pip install -e .`; tests pass from any directory; a comment on what changed.
